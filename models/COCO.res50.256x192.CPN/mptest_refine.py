...
from network_refine import Network
...
def test_net(tester, logger, dets, det_range):
    ...
    while img_start < det_range[1]:
        ...
        for test_id in range(0, len(test_data), batch_size):
            ...
            res, reg_x, reg_y = tester.predict_one([feed.transpose(0, 2, 3, 1).astype(np.float32)])[0]
            ...
            for test_image_id in range(start_id, end_id):
                ...
                #     y -= border
                #     x -= border
                #     x = max(0, min(x, cfg.output_shape[1] - 1))  # use later?
                #     y = max(0, min(y, cfg.output_shape[0] - 1))
                #     cls_skeleton[test_image_id, w, :2] = (x*4+2+reg_x[test_image_id-start_id,y,x,w], y*4+2+reg_y[test_image_id-start_id,y,x,w])
                #     cls_skeleton[test_image_id, w, 2] = r0[w, int(round(y) + 1e-10), int(round(x) + 1e-10)]


                # use original + postprocess
                for w in range(cfg.nr_skeleton):
                    lb = dr[w].argmax()
                    y, x = np.unravel_index(lb, dr[w].shape)
                    dr[w, y, x] = 0
                    ...
                    x = max(0, min(x, cfg.output_shape[1] - 1))
                    y = max(0, min(y, cfg.output_shape[0] - 1))
                    cls_skeleton[test_image_id, w, :2] = (x * 4 + 2 + reg_x[test_image_id-start_id,int(y),int(x),w],
                                                          y * 4 + 2 + reg_y[test_image_id-start_id,int(y),int(x),w])
                    cls_skeleton[test_image_id, w, 2] = r0[w, int(round(y) + 1e-10), int(round(x) + 1e-10)]
