...
def joints_heatmap_gen(data, label, tar_size=cfg.output_shape, ori_size=cfg.data_shape, points=cfg.nr_skeleton,
    ...
    ret_x = np.zeros((len(data), points, tar_size[0], tar_size[1]), dtype='float32')
    ret_y = np.zeros((len(data), points, tar_size[0], tar_size[1]), dtype='float32')
    xs, ys = np.arange(tar_size[1]), np.arange(tar_size[0])
    ...
    for i in range(len(ret_x)):
        for j in range(points):
            x, y = label[i][j << 1], label[i][j << 1 | 1]
            if x < 0 or y < 0:
                continue
            y = min(y, ori_size[0] - 1)
            x = min(x, ori_size[1] - 1)
            mask = ((x-xs)**2+(y-ys)**2)<=(gaussian_kernel[0])**2
            ret_x[i][j] = (x-xs)*mask
            ret_y[i][j] = (y-ys)*mask
            pass
...
def Preprocessing(d, stage='train'):
    ...
    if stage == 'train':
        ...
        heatmaps7_x, heatmaps7_y = joints_heatmap_gen(imgs, labels, cfg.output_shape, cfg.data_shape, return_valid=False,
        ...
        return [imgs.astype(np.float32).transpose(0, 2, 3, 1),
                ...
                heatmaps7_x.astype(np.float32).transpose(0, 2, 3, 1),
                heatmaps7_y.astype(np.float32).transpose(0, 2, 3, 1),
