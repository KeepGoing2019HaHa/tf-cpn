...
from tfflat.base_refine import ModelDesc, Trainer
...
def create_global_net(blocks, is_training, trainable=True):
    ...
    for i, block in enumerate(reversed(blocks)):
        ...
        with slim.arg_scope(resnet_arg_scope(bn_is_training=is_training)):
            ...
            # regress_x = slim.conv2d(tmp, cfg.nr_skeleton, [3, 3], trainable=trainable, weights_initializer=initializer,
            #                         padding='SAME', activation_fn=None, scope='regressX/res{}'.format(5-i))
            # regress_y = slim.conv2d(tmp, cfg.nr_skeleton, [3, 3], trainable=trainable, weights_initializer=initializer,
            #                         padding='SAME', activation_fn=None, scope='regressY/res{}'.format(5-i))
...
def create_refine_net(blocks, is_training, trainable=True):
    ...
    with slim.arg_scope(resnet_arg_scope(bn_is_training=is_training)):
        ...
        res = slim.conv2d(refine_fm, cfg.nr_skeleton, [3, 3],
            ...
            padding='SAME', activation_fn=None,
            scope='refine_out')
        regress_x = slim.conv2d(refine_fm, cfg.nr_skeleton, [3, 3], trainable=trainable, weights_initializer=initializer,
                                padding='SAME', activation_fn=None, scope='regressX')
        regress_y = slim.conv2d(refine_fm, cfg.nr_skeleton, [3, 3], trainable=trainable, weights_initializer=initializer,
                                padding='SAME', activation_fn=None, scope='regressY')
    return res, regress_x, regress_y

class Network(ModelDesc):
    def make_data(self):
        from COCOAllJoints import COCOJoints
        from dataset_refine import Preprocessing
    ...
    def make_network(self, is_train):
        if is_train:
            ...
            # label15_x = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            # label15_y = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            # label11_x = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            # label11_y = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            # label9_x = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            # label9_y = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            label7_x = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            label7_y = tf.placeholder(tf.float32, shape=[cfg.batch_size, *cfg.output_shape, cfg.nr_skeleton])
            valids = tf.placeholder(tf.float32, shape=[cfg.batch_size, cfg.nr_skeleton])
            # labels = [label7_x, label7_y]
            # labels.reverse() # The original labels are reversed. For reproduction of our pre-trained model, I'll keep it same.
            self.set_inputs(image, label7_x, label7_y, valids)
        else:
        ...
        refine_out, reg_x, reg_y = create_refine_net(global_fms, is_train)

        # make loss
        if is_train:
            refine_loss = tf.reduce_mean(tf.square(reg_x - label7_x) + tf.square(reg_y - label7_y), (1,2)) * tf.to_float((tf.greater(valids, 0.1)))
            refine_loss = tf.reduce_mean(refine_loss)
            ...
            self.set_outputs((refine_out, reg_x, reg_y))
